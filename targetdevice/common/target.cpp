...
{
    if(!m_bGtiPort) iRoute = MAX_CP_NODE - 1;
...
bool CTarget::GetHello(std::string &output)
{
	char            buf[1024]   = "at@rk:helloworld()";
	char            resp[GTI_RESPONSE_SIZE];
	unsigned short  size        = GTI_RESPONSE_SIZE;
	int i =0;
	LOGER((CLogger::DEBUG_DUT,"GetHello()\r\n"));
	if(!ConnectRouter(2)) return false;
	while (i<5)
	{
		i++;
		if (gti_generic(0, strlen(buf), buf, &size, resp, 0) && (strstr(resp, "ERR") == 0))
		...
		{
			//LOGER((CLogger::DEBUG_DUT,"--- mode_switch_in_progress!=FALSE ------"));
			LOGER((CLogger::DEBUG_DUT,"-- GetHello():fail"));
			return false;
		}
	}
	LOGER((CLogger::DEBUG_DUT,"-- GetHello():%s",output.c_str()));
    return true;
}

bool CTarget::StartTest(std::string &output)
{
	//char            buf[1024]   = "at@rk:ipc_request(\"CMD:{PCBA_START}\")";
	char            buf[1024]   = "at@rk:ipc_query(\"127.0.0.1\",56506,\"CMD:{PCBA_START}\")";
	char            resp[GTI_RESPONSE_SIZE];
	unsigned short  size        = GTI_RESPONSE_SIZE;
	int i =0;
	LOGER((CLogger::DEBUG_DUT,"StartTest()\r\n"));
	if(!ConnectRouter(2)) return false;
	while (i<5)
	{
		i++;
		...
		{
			//LOGER((CLogger::DEBUG_DUT,"--- mode_switch_in_progress!=FALSE ------"));
			LOGER((CLogger::DEBUG_DUT,"-- StartTest():fail"));
			return false;
		}
	}
	LOGER((CLogger::DEBUG_DUT,"-- StartTest():%s",output.c_str()));
    return true;
}

bool CTarget::QueryStatus(std::string &output)
{
	//char            buf[1024]   = "at@rk:ipc_request(\"QUERY:{bt,wifi}\")";
	char            buf[1024]   = "at@rk:ipc_query(\"127.0.0.1\",56506,\"QUERY:{bt,wifi}\")";
	char            resp[GTI_RESPONSE_SIZE];
	unsigned short  size        = GTI_RESPONSE_SIZE;
	int i =0;
	LOGER((CLogger::DEBUG_DUT,"QueryStatus()\r\n"));
	if(!ConnectRouter(2)) return false;
	if (gti_generic(0, strlen(buf), buf, &size, resp, 0) && (strstr(resp, "ERR") == 0))
	{
		output = resp;
	...
	{
		//LOGER((CLogger::DEBUG_DUT,"--- mode_switch_in_progress!=FALSE ------"));
		LOGER((CLogger::DEBUG_DUT,"-- QueryStatus():fail"));
		return false;
	}
	LOGER((CLogger::DEBUG_DUT,"-- QueryStatus():%s",output.c_str()));
    return true;
}

bool CTarget::EndTest()
{
	//char            buf[1024]   = "at@rk:ipc_request(\"CMD:{PCBA_END}\")";
    char            buf[1024]   = "at@rk:ipc_query(\"127.0.0.1\",56506,\"CMD:{PCBA_END}\")";
	char            resp[GTI_RESPONSE_SIZE];
	unsigned short  size        = GTI_RESPONSE_SIZE;
	int i =0;
	LOGER((CLogger::DEBUG_DUT,"EndTest()\r\n"));
	if(!ConnectRouter(2)) return false;
	while (i<5)
	{
		i++;
        if (!gti_generic(0, strlen(buf), buf, &size, resp, 0) || (strstr(resp, "ERR") != 0)) {
            LOGER((CLogger::DEBUG_DUT,"-- EndTest():fail"));
            return false;
        }
	}
    return true;
	//LOGER((CLogger::DEBUG_DUT,"-- EndTest():%s",output.c_str()));
}