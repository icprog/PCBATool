...
BOOL CDatabaseSettingDlg::OnInitDialog()
{
    ...
    /*Database key map **/
    for(int i = FLAG_WVKEY;i < FLAG_WVCNT; i++ ) {
        SetDlgItemText(IDC_EDIT_TABLE_WVKEY + i - FLAG_WVKEY,m_Configs.strItemName[i - FLAG_WVKEY].c_str());
    }
#if 0
...
BOOL CDatabaseSettingDlg::SaveConfig()
{
    ...
    for(int i = FLAG_WVKEY;i < FLAG_WVCNT; i++ ) {
        ...
        GetDlgItemText(IDC_EDIT_TABLE_WVKEY + i - FLAG_WVKEY,szItem,64);
        m_Configs.strItemName[i - FLAG_WVKEY ] = szItem;
    }
...
void CDatabaseSettingDlg::OnBnClickedButtonCreate()
{
	...
	wstring strWvKey;
	if(m_WvToolDB.GetWVkey(0,0,0,strWvKey)) {
	}
#endif
}
